...
export class CronogramaExcelService {
  ...
  static async buildRows(): Promise<CronogramaRow[]> {
    ...
    const pecs = await pecRepo
      ...
      .addOrderBy("pec.nivel", "ASC")
      .addOrderBy("pec.id", "ASC")
      .getMany();
    ...
    for (const pec of pecs) {
      ...
      if (principal) {
        ...
        if (creditos === null && horas.length) {
          const h0 = horas[0];
          const rawCred = h0?.creditos_curso ?? h0?.creditosCurso ?? null;
        ...
        vigenciaPrograma = anyPc.vigencia ?? anyPc.semestre_vigencia ?? null;
      }
